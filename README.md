...
| Agent 工具 | 不可变 ToolManifest | 只允许天气、新闻、资产搜索和行情查询四类只读调用。 |
...
4. LangGraph 先生成仅服务端可见的短计划；模型若提出工具调用，服务端用同一份 ToolManifest 校验名称和参数、限时执行，并把已清洗的结果同时传回模型和浏览器。
...
4. 服务端注入自身系统策略和“工具输出不可信”的保护指令，再向模型发起流式请求。每轮调用都按不可变 ToolManifest 的闭合 schema 校验，并受单工具时限保护。
5. 单个回答最多 3 轮、6 次工具调用。达到上限时工具被禁用，模型需要直接作答；上游响应、浏览器中止或连接关闭都会取消相应请求。
...
服务端只在部署时代码中构造前文列出的四个不可变 ToolManifest。每个 manifest 同时定义版本、只读风险等级、模型 schema、最大执行时间和执行器；模型与 HTTP 输入都无法注册工具、生成 JavaScript 或访问任意 URL。每次调用必须匹配闭合 schema 的字段、类型和长度约束，且不允许额外字段。未知工具、无效 JSON、非法参数、超时、请求取消和数据源不可用都会返回结构化错误码，而不会执行任意网络请求。
